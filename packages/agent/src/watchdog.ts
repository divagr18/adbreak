...
export interface WatchdogConfig {
  ...
  stallFloorMs: number;
  /** Multiple of a step's rolling p95 that counts as stalled. */
  stallP95Multiple: number;
...
/** p95 of past durations for a step, in ms. Null when there is no history. */
export function p95(durationsMs: number[]): number | null {
...
export function stallThresholdMs(
  ...
  const base = p95(history);
  const fromHistory = base === null ? cfg.stallFloorMs : Math.max(cfg.stallFloorMs, base * cfg.stallP95Multiple);
...
export function historyFrom(
  runs: { steps: { step: string; durationMs: number }[] }[],
  keep = 20,
): Map<string, number[]> {
